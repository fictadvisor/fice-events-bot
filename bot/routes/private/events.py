...
async def select_event(callback: CallbackQuery, callback_data: SelectEvent, session: AsyncSession) -> None:
    ...
    request = await request_repository.find_one(RequestFilter(user_id=callback.from_user.id))
...
async def register_event(callback: CallbackQuery, state: FSMContext, callback_data: RegisterEvent, session: AsyncSession) -> None:
    if callback.message is None:
        return

    request_repository = RequestRepository(session)
    request = await request_repository.find_one(RequestFilter(user_id=callback.from_user.id, event_id=callback_data.event_id))
...
async def reset_form(callback: CallbackQuery, state: FSMContext, callback_data: ConfirmData, session: AsyncSession) -> None:
    if callback.message is None:
        return
    ...
    if callback_data.select == Select.YES:
        answers = await answer_repository.find(AnswerFilter(request_id=data.get("request_id")))
        question = await question_repository.find_one(QuestionFilter(event_id=data.get("event_id")), offset=len(answers))
        if question is None:
            await state.set_state(RegistrationForm.confirm)
            await callback.message.edit_text(await CONFIRM_REGISTRATION.render_async(answers=answers), reply_markup=CONFIRM_KEYBOARD)
            return

        await state.update_data(offset=len(answers)+1)
        await callback.message.edit_text(question.text)
    else:
        await answer_repository.delete_many(AnswerFilter(request_id=data.get("request_id")))
        question = await question_repository.find_one(QuestionFilter(event_id=data.get("event_id")))
        if question is None:
            return
...
async def answer_question(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    previous_question = await question_repository.find_one(
        QuestionFilter(event_id=data.get('event_id')),
        offset=offset-1
    )
    ...
    if question is None:
        answers = await answer_repository.find(
            AnswerFilter(request_id=data.get("request_id")),
            options=(
...
async def confirm_registration(callback: CallbackQuery, state: FSMContext, callback_data: ConfirmData, session: AsyncSession) -> None:
    if callback.message is None:
        return
    ...
    request_repository = RequestRepository(session)
    request = await request_repository.find_one(RequestFilter(user_id=callback.from_user.id, event_id=data.get("event_id")))
    if request is None:
        return
